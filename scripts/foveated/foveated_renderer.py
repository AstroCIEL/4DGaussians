...
def _process_single_image(
    ...
    cv2.imwrite(output_path, foveated)
...
def _process_folder(
    ...
    foveated_enabled: bool,
    fixation: Tuple[float, float],
    image_extensions: Tuple[str, ...] = (".png", ".jpg", ".jpeg", ".bmp", ".tiff", ".tif"),
) -> None:
    """处理文件夹中的所有图片"""
    ...
    print(f"完成！共处理 {len(image_files)} 张图片，输出到: {output_dir}")


def _build_argparser() -> ArgumentParser:
    parser = ArgumentParser(description="Simulate foveated rendering on images or folders.")
    parser.add_argument("input", type=str, help="输入图片路径或文件夹路径")
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="输出图片路径或文件夹路径（单张图片时默认在文件名后加 _foveated，文件夹时默认为输入文件夹名_foveated）",
    )
    parser.add_argument(
        "--tile_size",
        type=int,
...
def main() -> None:
    ...
    if input_path.is_file():
        ...
        # 文件夹批量处理
        if args.output is None:
            output_dir = str(input_path) + "_foveated"
        else:
