#!/usr/bin/env python3
"""
分析4DGS中各个场景test split view中各tile需要处理的高斯球数量统计

用法:
    # 分析单个场景
    python scripts/analyze_tile_gaussians.py --model_path output/dynerf/bouncingballs --source_path data/dynerf/bouncingballs --iteration 30000
    
    # 分析整个数据集的所有场景
    python scripts/analyze_tile_gaussians.py --dataset dynerf --base_dir output --iteration 30000
"""
...
def compute_tile_gaussians(means2D, radii, image_width, image_height, tile_size=TILE_SIZE):
    ...
    Returns:
        tile_gaussians: dict，key为(tile_x, tile_y)，value为该tile的高斯球数量
    ...
    # 对于每个可见的高斯球，计算它覆盖的tile
    for i in range(len(visible_means2D)):
        x, y = visible_means2D[i, 0].item(), visible_means2D[i, 1].item()
        radius = visible_radii[i].item()
        
        # 计算高斯球在屏幕空间的边界框
        # 使用radius作为近似（实际应该考虑椭圆，这里简化处理）
        ...
        # 计算覆盖的tile范围
        tile_min_x = int(min_x // tile_size)
        tile_max_x = int(max_x // tile_size)
        tile_min_y = int(min_y // tile_size)
        tile_max_y = int(max_y // tile_size)
        ...
        for tile_x in range(tile_min_x, tile_max_x + 1):
            for tile_y in range(tile_min_y, tile_max_y + 1):
                tile_gaussians[(tile_x, tile_y)] += 1
    
    # 转换为统计信息
    tile_counts = list(tile_gaussians.values())
    
    tile_stats = {
        "num_tiles": len(tile_gaussians),
        "total_tiles": num_tiles_x * num_tiles_y,
        "total_gaussians": len(means2D),
        "visible_gaussians": len(visible_means2D),
    ...
    if len(tile_counts) > 0:
        tile_stats["max_gaussians_per_tile"] = int(np.max(tile_counts))
        tile_stats["min_gaussians_per_tile"] = int(np.min(tile_counts))
        tile_stats["mean_gaussians_per_tile"] = float(np.mean(tile_counts))
        tile_stats["median_gaussians_per_tile"] = float(np.median(tile_counts))
        tile_stats["std_gaussians_per_tile"] = float(np.std(tile_counts))
    else:
    ...
    return dict(tile_gaussians), tile_stats


def analyze_scene_tile_gaussians(model_path, source_path, iteration, dataset_type=None, frame_idx=None):
    """
    分析单个场景的tile高斯球统计
    ...
    for idx, view in zip(view_indices, tqdm(views_to_analyze, desc="分析tile高斯球统计")):
        ...
        view_stats.append({
            ...
            "visible_gaussians": tile_stats["visible_gaussians"],
            "tile_stats": tile_stats,
            "tile_gaussians": {f"{k[0]}_{k[1]}": v for k, v in tile_gaussians.items()}  # 转换为可序列化的格式
        })
...
def save_scene_results(summary, output_dir):
    ...
    # 保存JSON结果
    json_path = os.path.join(scene_output_dir, "tile_gaussians_stats.json")
...
def plot_scene_results(summary, output_dir):
    ...
    fig.suptitle(f'Tile Gaussians Analysis: {scene_name}', fontsize=16, fontweight='bold')
    ...
    axes[0, 0].set_title('Max Gaussians per Tile per View')
    axes[0, 0].legend()
    ...
    axes[0, 1].set_title('Mean Gaussians per Tile per View')
    axes[0, 1].legend()
    ...
    axes[1, 0].set_title('Min Gaussians per Tile per View')
    axes[1, 0].legend()
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    plt.tight_layout()
    plot_path = os.path.join(scene_output_dir, "tile_gaussians_analysis.png")
...
def aggregate_dataset_results(base_dir, dataset_name, iteration, output_dir, frame_idx=None):
    ...
    for scene in scenes:
        ...
        try:
            summary = analyze_scene_tile_gaussians(model_path, source_path, iteration, frame_idx=frame_idx)
...
def generate_dataset_summary(all_summaries, dataset_name, output_dir):
    ...
    fig.suptitle(f'Tile Gaussians Analysis - Dataset: {dataset_name}', fontsize=16, fontweight='bold')
    ...
    ax.set_title('Gaussians per Tile Statistics (Max, Mean, Min) by Scene', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plot_path = os.path.join(dataset_output_dir, f"{dataset_name.replace('/', '_')}_tile_gaussians_aggregated.png")
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close()
    ...
    json_path = os.path.join(dataset_output_dir, f"{dataset_name.replace('/', '_')}_tile_gaussians_aggregated.json")
    with open(json_path, 'w') as f:
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Tile Gaussians Analysis Script")
    ...
    if args.dataset:
        # 分析整个数据集
        output_dir = os.path.join(args.output_dir, args.dataset, "tile_gaussians_analysis")
        ...
        # 分析单个场景
        output_dir = os.path.join(args.output_dir, "tile_gaussians_analysis")
        os.makedirs(output_dir, exist_ok=True)
        summary = analyze_scene_tile_gaussians(args.model_path, args.source_path, args.iteration, frame_idx=args.frame_idx)
        save_scene_results(summary, output_dir)
        plot_scene_results(summary, output_dir)
    else:
