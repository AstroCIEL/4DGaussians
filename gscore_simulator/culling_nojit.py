...
def cull_and_convert(
    ...
    Maximizes performance by processing mathematical operations for all Gaussians at once using NumPy array operations.
    """
    with torch.no_grad():
        ...
        # 2-4) culled gaussians list (stored in G_List along with culling results)
        ...
        # Get min/max eigenvectors and radii from covariance
        ...
        tx_min = torch.div(mins[:,0], tile_size, rounding_mode='trunc')  # Overlapping tile x start coordinate (N,)
        tx_max = torch.div(maxs[:,0], tile_size, rounding_mode='trunc')  # Overlapping tile x end coordinate (N,)
        ty_min = torch.div(mins[:,1], tile_size, rounding_mode='trunc')  # Overlapping tile y start coordinate (N,)
        ty_max = torch.div(maxs[:,1], tile_size, rounding_mode='trunc')  # Overlapping tile y end coordinate (N,)
        # Create AABB for each Gaussian and find overlapping tile start coordinates
        ...
        # ======================================================================
        # <<< Logic Modification Start: Generate Final Return Value >>>
        # ======================================================================
        ...
        all_zero_mask = (bitmap_aabb_test.sum(dim=1) == 0)  # Check if the sum of each row is 0
        ...
        radii_max = radii_obb_test.max().values  # Largest axis
        radii_min = radii_obb_test.min().values  # Smallest axis
        ...
        '''
        # Calculate number of rows where all elements in bitmap are 0
        ...
        # 5. Create final data structure for GSU input
        print("Reformatting data for GSU...")
        # 5-1. Reconstruct intersecting tile list based on Gaussian ID.
        depths = p_view[:, 2]
...
def _compute_cov3d_vectorized(scales: torch.Tensor, rotations: torch.Tensor) -> torch.Tensor:
    """Calculates 3D covariance matrices for all Gaussians in bulk.
    Args:
...
def shape_analysis(cov2d_t):
    ...
    R1 = torch.clamp(R1, min=0.5, max=64)  # Clamped to twice the tile size
    R2 = torch.clamp(R2, min=0.5, max=64)

    # N_total = cov2d_t.shape[0]
    # print(f"Number of Spiky Gaussians: {num_spiky}, Total Gaussians: {N_total}")
...
def _get_obb_corners(center, axes, radii):
    """Calculate the 4 corner coordinates of the OBB"""
    # radii_2d = radii.unsqueeze(1).expand(-1, 2) -> Used when radii is a scalar
...
def _get_tile_corners(tx, ty, tile_size, device=None):
    """Return 4 corner coordinates of the tile as torch.Tensor"""

    min_x, min_y = tx, ty
    max_x, max_y = min_x + tile_size, min_y + tile_size
    # 3) Stack into (T,4,2) shape
...
def _get_subtile_corners(tx, ty, stx, sty, tile_size, subtile_res, device=None):
    """Return 4 corner coordinates of the subtile as torch.Tensor"""
...
def check_sat_intersection(
    ...
    """
    SAT intersection test without for-loop (M×T → (M,T) boolean mask)
    """
    ...
    print(f"    chunk size (tiles processed at once): {chunk_size}")
    for chunk_id in tqdm(range(n_chunks), desc="SAT chunks"):
        ...
        '''
        if chunk_id == 20:
            # e.g., 0-th Gaussian and 0-th tile
...
def vectorize_candidate_generation(num_culled, tx_min, tx_max, ty_min, ty_max):
    ...
    # Immediately return empty tensor if no Gaussians to process
    if num_culled == 0:
        return torch.empty((0, 3), dtype=torch.long, device=device)

    # 1. Calculate max tile width and height each Gaussian occupies
    ...
    # 2. Create local offset grid (0,0), (0,1), ..., (h,w)
    # size: (max_height, max_width)
    dx = torch.arange(max_width, device=device)
    dy = torch.arange(max_height, device=device)
    # 'ij' indexing: first index y (row), second index x (column)
    grid_y, grid_x = torch.meshgrid(dy, dx, indexing='ij')

    # 3. Generate all tile candidates for all Gaussians using broadcasting
    # tx_min[:, None, None] expands (N,) -> (N, 1, 1) to allow operations with
    # the (max_height, max_width) size grid
    ...
    # 5. Select only valid candidates and combine into final result
    # Expand Gaussian index g to the same size as the mask using broadcasting
    ...
    # Concatenate 1D tensors to create final result of shape (M, 3)
...
def calculate_bitmap_obb(
    ...
    """
    For (gauss, tile) pairs that passed SAT intersection,
    calculates 0/1 bitmap by subtile units instead of pixels.
    Processes (gauss, tile) pairs in batches to prevent memory explosion.
    """
    device = gauss_centers.device
    dtype = gauss_centers.dtype

    # precompute: subtile position offset
    n_sub_x = math.ceil(tile_size / subtile_size)
    n_sub_y = math.ceil(tile_size / subtile_size)
    S = n_sub_x * n_sub_y

    # Flatten subtiles inside tile
    ...
    # Actual intersection pairs (gauss_idx, tile_idx) from SAT result
    ...
    for start in tqdm(range(0, total_pairs, batch_size), desc="Calculating OBB bitmap", ncols=100):
        ...
        # --- Step 1: Calculate OBB AABB ---
        # Calculate OBB corners for each Gaussian first.
        ...
        # Find min/max coordinates of AABB enclosing OBB. (Calculate once per Gaussian within batch)
        obb_mins = obb_corners.min(dim=1).values  # (P, 2)
        obb_maxs = obb_corners.max(dim=1).values  # (P, 2)

        # --- Step 2: Simple overlap check with subtile AABB ---
        # Calculate start coordinates for each subtile
        tile_xy = tile_origins[tile_idx_b]
        subtile_origins = tile_xy[:, None, :] + block_offsets[None, :, :] # (P, S, 2)

        # Calculate min/max coordinates for each subtile AABB
        ...
        # AABB overlap check using broadcasting (much faster than SAT)
        # Check if Gaussian AABB and subtile AABB overlap
        ...
        # If both axes overlap, determine as finally overlapping
...
def calculate_bitmap_aabb(
    ...
    with torch.no_grad():
        ...
        B = 2000
        total = mins.shape[0]   # Number of AABB Gaussians
